...
from services.scoring import calculate_experience_score, calculate_education_score, get_final_score
...
async def evaluate(
    ...
    for file in files:
        ...
        skills_score = get_semantic_match_score(jd_data.get("skills", []), resume_data.get("skills", []))
        tools_score = get_semantic_match_score(jd_data.get("tools", []), resume_data.get("tools", []))
        exp_score = calculate_experience_score(resume_data.get("experience_years", 0), jd_data.get("experience_years", 0))
        edu_score = calculate_education_score(resume_data.get("education", ""), jd_data.get("education", ""))
